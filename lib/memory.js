...
// FIX: PAKAI UPSERT BIAR GA GAGAL DI SERVERLESS
export async function upsertMemory(userId, key, value) {
  try {
    ...
    console.error("Gagal upsert memory:", err.message);
  }
}
...
// UDAH DIUPDATE: HAPUS DUPLIKAT KEY + PRIORITASKAN
export function formatMemoriesForPrompt(memories) {
  if (memories.length === 0) return "Belum ada memory tersimpan untuk user ini.";

  const map = new Map();
  for(const m of memories){
    if(!map.has(m.key)) map.set(m.key, m.value); // ambil yg paling baru
  }
  ...
  if(lain.length) result += lain.slice(0,6).join("\n"); // max 6 biar hemat token
