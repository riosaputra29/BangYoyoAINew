...
- "ganti nama jadi Budi", "panggil aku Budi", "namaku Budi" -> key nya tetap "nama"
- Kalau tidak ada fakta baru yang layak disimpan, balas: []
- JANGAN simpan informasi sensitif: kesehatan, agama, politik, keuangan.`;
...
// FALLBACK REGEX BIAR AMAN KALAU AI LIMIT
function extractFactsFallback(userMessage) {
  const facts = [];

  // TANGKAP "GANTI NAMA JADI" / "PANGGIL AKU"
  const nameMatch = userMessage.match(/(?:ganti nama|panggil aku|namaku|nama saya)\s+(?:jadi\s+)?([A-Za-z\s]{2,20})/i);
  if (nameMatch) facts.push({ key: "nama", value: nameMatch[1].trim() });
  ...
  const jobMatch = userMessage.match(/(?:kerja|bekerja|jualan|usaha)\s+(?:sebagai\s+)?(.{3,50})/i);
  if (jobMatch) facts.push({ key: "pekerjaan", value: jobMatch[1].trim() });

  const hobbyMatch = userMessage.match(/(?:hobi|suka)\s+(.{3,50})/i);
  if (hobbyMatch) facts.push({ key: "hobi", value: hobbyMatch[1].trim() });
...
export async function extractAndSaveFacts(userId, userMessage) {
  ...
  for (const fact of facts) {
    // NORMALISASI: semua variasi nama jadi key "nama"
    let key = fact.key;
    if(['panggilan','nama_panggilan','nickname'].includes(key)) key = "nama";

    await upsertMemory(userId, key, fact.value);
  }
}
